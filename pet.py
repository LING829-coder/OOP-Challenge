class Pet:
    ...
    def get_status(self):
        ...
        if self.hunger <= 3 and self.energy >= 7 and self.happiness >= 7:
            print(f"{self.name} is in excellent condition!")
        elif self.hunger >= 7 and self.energy <= 3 and self.happiness <= 3:
            print(f"{self.name} is in poor condition and needs immediate care!")
        else:
            print(f"{self.name} is doing okay but could use some attention.")
...
# show the pet's status and tricks
my_pet.get_status()
my_pet.show_tricks()
